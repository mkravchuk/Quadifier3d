...
namespace utils
{
    namespace sse
    {
        ...
        namespace _4
        {
            ...
            */
            inline __m128i v4_sort_float(__m128& v)
            {
            ...
            void testSort4xOneTime(int* a, int valuesCount)
            {
                __m128i am;
                for (int i = 0; i < valuesCount; i++) am.m128i_i32[i] = a[i];
                ...
                int  si[4] = { sim.m128i_i32[0], sim.m128i_i32[1], sim.m128i_i32[2], sim.m128i_i32[3] };
                int aSortedIndexed[4];
                for (int i = 0; i < valuesCount; i++)
                {
                    aSortedIndexed[i] = ami.m128i_i32[i];
                }
                ...
                for (int i = 0; i < valuesCount; i++)
                {
                    if (aSorted[i] != i) ok = false;
                    if (aSortedIndexed[i] != i) ok = false;
                }

                cout << (ok ? "ok  " : "fail") << "      ";
                utils::strings::printArray("a", a, valuesCount);
                utils::strings::printArray("     sorted a", aSorted, valuesCount);
                utils::strings::printArray("     sorted a (indexeing version)", aSortedIndexed, valuesCount);
                utils::strings::printArray("     indexes", si, valuesCount);
                cout << endl;
            }

            void v4_sort_ints_sse(__m128i& v, __m128i& sortedIndexes, int valuesCount)
            {
            }

            void testSort1234()
            {
                cout << "testing sort of 2 integers ..." << endl;
                for (int i0 = 0; i0 < 2; i0++)
                    for (int i1 = 0; i1 < 2; i1++)
                    {
                        if (i0 == i1) continue;
                        int a[2] = { i0, i1 };
                        testSort4xOneTime(a, 2);
                    }
                cout << "testing sort of 3 integers ..." << endl;
                for (int i0 = 0; i0 < 3; i0++)
                    for (int i1 = 0; i1 < 3; i1++)
                        for (int i2 = 0; i2 < 3; i2++)
                        {
                            ...
                            int a[3] = { i0, i1, i2 };
                            testSort4xOneTime(a, 3);
                        }
                cout << "testing sort of 4 integers ..." << endl;
        ...
        void sqrt(Ds& data)
        {
            int count = data.size();
            if (count == 0) return;
            cout << "! error in utils::sse::sqrt :   not implemented yet!";
            assert(false && "not implemented yet!");
            throw;

            if (data.sizeofT() == 4) // float
            {

            }
            else if (data.sizeofT() == 8) // double
            {

            }
            else
            {
                cout << "! error in utils::sse::sqrt :   unsupported data type";
  ...
  static const ALIGN16_BEG Type _ps_##Name[4] ALIGN16_END = { Val, Val, Val, Val }
    ...
    xmm_mm_union u; u.mm[0]=mm0_; u.mm[1]=mm1_; xmm_ = u.xmm;      \
        ...
        */
        ...
        void __vectorcall sincos(__m128 x, __m128& _sin, __m128& _cos)
        {
            ...
            /* update the sign */
            _sin = _mm_xor_ps(xmm1, sign_bit_sin);
            _cos = _mm_xor_ps(xmm2, sign_bit_cos);
        }
    }
}
