...
static void glnvg__renderFlush(void* uptr, NVGcompositeOperationState compositeOperation)
{
	...
	if (gl->ncalls > 0) {
        // Mupoc - fix changing GL values after draw 2d text
	    // remember GL values
        bool save__BLEND = glIsEnabled(GL_BLEND);
        bool save__DEPTH_TEST = glIsEnabled(GL_DEPTH_TEST);
        bool save__CULL_FACE = glIsEnabled(GL_CULL_FACE);
        GLint save__CULL_FACE_MODE;
        glGetIntegerv(GL_CULL_FACE_MODE, &save__CULL_FACE_MODE);
        ...
        // Mupoc - fix changing GL values after draw 2d text
        // restore GL values
        if (glIsEnabled(GL_BLEND) != save__BLEND) if (save__BLEND) glEnable(GL_BLEND); else glDisable(GL_BLEND);
        if (glIsEnabled(GL_DEPTH_TEST) != save__DEPTH_TEST) if (save__DEPTH_TEST) glEnable(GL_DEPTH_TEST); else glDisable(GL_DEPTH_TEST);
        if (glIsEnabled(GL_CULL_FACE) != save__CULL_FACE) if (save__CULL_FACE) glEnable(GL_CULL_FACE); else glDisable(GL_CULL_FACE);
        glCullFace(save__CULL_FACE_MODE);
        glBlendFunc(GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA); // standard approach for blending colors
    }
