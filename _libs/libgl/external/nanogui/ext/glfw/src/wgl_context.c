...
{
    ////Mupoc - skip changing context if it is same
    static _GLFWwindow* current_window;
...
)
{
    ...
    //if (choisePixelFormat == GLFW_TRUE)
    {
        // to improve performance of opengl call to 'PixelFormat' we will preinit long opengl call  in 'main' - https://hero.handmade.network/forums/code-discussion/t/2503-%5Bday_235%5D_opengls_pixel_format_takes_a_long_time
    ...
    /*else
    {
        ...
        ////pixelFormat = GetPixelFormat(window->context.wgl.dc);
        memset(&pfd, 0, sizeof(PIXELFORMATDESCRIPTOR));
        pfd.nSize = sizeof(PIXELFORMATDESCRIPTOR);
        pfd.nVersion = 1; // shoudlb be always 1
        //pfd.dwFlags = PFD_DOUBLEBUFFER | PFD_DRAW_TO_WINDOW | PFD_SUPPORT_OPENGL | PFD_SUPPORT_COMPOSITION;// 32805;
        pfd.dwFlags = PFD_DOUBLEBUFFER | PFD_DRAW_TO_WINDOW | PFD_SUPPORT_OPENGL | PFD_GENERIC_ACCELERATED;
        pfd.iPixelType = PFD_TYPE_RGBA; // Specifies the type of pixel data. The following types are defined: PFD_TYPE_RGBA, PFD_TYPE_COLORINDEX
        //pfd.cColorBits = 32;        // Specifies the number of color bitplanes in each color buffer. For RGBA pixel types, it is the size of the color buffer, excluding the alpha bitplanes. For color-index pixels, it is the size of the color-index buffer.
        ...
        //pfd.cBlueShift = 16;         // Specifies the shift count for blue bitplanes in each RGBA color buffer.
        //pfd.cAlphaBits = 8;        // Alpha bitplanes are not supported.
        //pfd.cAlphaShift = 24;     // Alpha bitplanes are not supported.
        ...
        pfd.cAuxBuffers = 0;       // Auxiliary buffers are not supported.
        //pfd.iLayerType = 0;         //Ignored. Earlier implementations of OpenGL used this member, but it is no longer used.  //PFD_MAIN_PLANE; // PFD_MAIN_PLANE,  PFD_OVERLAY_PLANE,  PFD_UNDERLAY_PLANE
        //pfd.bReserved = 0;         // Specifies the number of overlay and underlay planes. Bits 0 through 3 specify up to 15 overlay planes and bits 4 through 7 specify up to 15 underlay planes.
        //pfd.dwLayerMask = 0;     // Ignored
        //pfd.dwVisibleMask = 0;   // Specifies the transparent color or index of an underlay plane. When the pixel type is RGBA, dwVisibleMask is a transparent RGB color value. When the pixel type is color index, it is a transparent index value. 
        //pfd.dwDamageMask = 0; // Ignored
        pixelFormat = ChoosePixelFormat(window->context.wgl.dc, &pfd);
