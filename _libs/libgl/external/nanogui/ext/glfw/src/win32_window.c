                              ...
                              const _GLFWfbconfig* fbconfig)
{
    ...
    if (ctxconfig->client != GLFW_NO_API)
    {
        ...
        {
            if (!_glfwCreateContextWGL(window, ctxconfig, fbconfig, GLFW_FALSE))
                return GLFW_FALSE;
            ...
            if (status == _GLFW_RECREATION_REQUIRED && wndconfig->recreateWindowAfterLoadExtensions == GLFW_TRUE)
            {
                ...
                if (!createWindow(window, wndconfig))
                    return GLFW_FALSE;
                if (!_glfwCreateContextWGL(window, ctxconfig, fbconfig, GLFW_TRUE))
                    return GLFW_FALSE;
            }
        }
        else
