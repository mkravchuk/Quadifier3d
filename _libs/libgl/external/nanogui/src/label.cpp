...
void Label::draw(NVGcontext *ctx, bool drawOnlyGui) {
    Widget::draw(ctx, drawOnlyGui);
    nvgFontFace(ctx, mFont.c_str());
    nvgFontSize(ctx, fontSize());
    //nvgFillColor(ctx, mColor);
    nvgFillColor(ctx, mEnabled ? mTheme->mTextColor : mTheme->mDisabledTextColor); // Mupoc - always read color from theme
    if (mFixedSize.x() > 0) {
        nvgTextAlign(ctx, NVG_ALIGN_LEFT | NVG_ALIGN_TOP);
