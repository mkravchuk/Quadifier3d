...
void Screen::drawWidgets(bool drawOnlyGui)
{
    if (!mVisible)
        return;

    double elapsed = glfwGetTime() - mLastInteraction;
    //std::cout << "nanogui.drawWidgets()   "<<(drawOnlyGui ? "skip" : "paint") << "     elapsed=" << elapsed << std::endl;
//if (!drawOnlyGui)
//std::cout << "nanogui.drawWidgets()   "<<(drawOnlyGui ? "skip" : "paint") << "     elapsed=" << elapsed << std::endl;

    //if (drawOnlyGui && elapsed < 0.0002)
    //{
    //    std::cout << "   ignoring" << std::endl;
    //    return; // skip repaiting very ofthen
    //}
    ...
    //mMouseFocus = true;
    draw(mNVGContext, drawOnlyGui);


    // if 1 second use do nothing - try to provide tooltip
    if (elapsed > 0.99f)
    {
        /* Draw tooltips */
...
{
    markDity(); // if mouse click - make dirty anyway
...
{
    markDity(); // if keyboard click - make dirty anyway
    mLastInteraction = glfwGetTime();
    try
    {
...
bool Screen::dropCallbackEvent(int count, const char** arg)
{
    markDity(); // if drop - make dirty anyway
    std::vector<std::string> filenames(count);
    for (int i = 0; i < count; ++i)
        filenames[i] = arg[i];

    return dropEvent(filenames);
}
