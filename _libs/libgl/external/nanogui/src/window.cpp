...
void Window::draw(NVGcontext *ctx, bool drawOnlyGui) {
    ...
    /* Draw window */
    nvgSave(ctx);
    //nvgGlobalCompositeBlendFuncSeparate(ctx, NVG_ZERO, NVG_ZERO, NVG_ZERO, NVG_ZERO);
    nvgBeginPath(ctx);
    nvgRoundedRect(ctx, mPos.x(), mPos.y(), mSize.x(), mSize.y(), cr);

    nvgFillColor(ctx, mMouseFocus ? mTheme->mWindowFillFocused
                                  : mTheme->mWindowFillUnfocused);
    nvgFill(ctx); // clear background for window - this required other widgets to redraw

    ds *= 1.5; // compensate shadow for layers - layers draws a bit to light shadow, so we have to compensate somehow
    ...
    else
    {
        ...
        NVGpaint shadowPaint = nvgBoxGradient(
            ...
            //Color(955, 0, 0, 255), mTheme->mTransparent);
        //mTheme->mDropShadow, Color(0,0,0,0));
        //nvgGlobalCompositeBlendFunc(ctx, 
        //nvgReset(ctx);
        ...
        //nvgGlobalCompositeBlendFunc(ctx, NVG_ONE, NVG_ZERO);
        //nvgGlobalCompositeBlendFuncSeparate(ctx, NVG_ZERO, NVG_ZERO, NVG_ZERO, NVG_ZERO);
