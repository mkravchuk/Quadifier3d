...
void GridLayout::computeLayout(NVGcontext *ctx, const Widget *widget, std::vector<int> *grid) const {
    ...
    for (auto w : widget->children())
    {
        ...
        maxResolutionIndex = std::max(maxResolutionIndex, w->GridLayoutResolutionIndex());
    }

    auto setmax = [&grid, &axis1, &axis2, &ctx](int i1, int i2, const Widget *w)
    {
    ...
    {
        std::vector<int> resCounts(maxResolutionIndex + 1, 0);
        for (auto w : widget->children())
        {
            if (!w->visible()) continue;
            int index = w->GridLayoutResolutionIndex();
            resCounts[index]++;
        }
        int maxItemsInRes = *std::max_element(resCounts.begin(), resCounts.end());
        grid[axis1].clear(); grid[axis1].resize(maxResolutionIndex+1, 0);
        grid[axis2].clear(); grid[axis2].resize(maxItemsInRes, 0);
        std::vector<int> resNextIndex(maxResolutionIndex + 1, 0);
        for (auto w : widget->children())
        {
            if (!w->visible()) continue;
            int i1 = w->GridLayoutResolutionIndex();
            int i2 = resNextIndex[i1];
            resNextIndex[i1]++;
            setmax(i1, i2, w);
        }
    }
...
void GridLayout::performLayout(NVGcontext *ctx, Widget *widget) const {
    ...
    auto setpos = [&](int i1, int i2, const Vector2i& pos, Widget *w)
    {
        Vector2i ps = w->preferredSize(ctx);
        Vector2i fs = w->fixedSize();
        Vector2i targetSize(
    ...
    {
        std::vector<int> resNextIndex(maxResolutionIndex + 1, 0);
        std::vector<Vector2i> poss(maxResolutionIndex + 1, start);
        for (int i1 = 1; i1 < poss.size(); i1++)
        {
            poss[i1](axis1) += poss[i1-1](axis1)- start(axis1) + grid[axis1][i1-1] + mSpacing[axis1];
        }
        ...
        for (auto w : widget->children())
        {
            ...
            int i2 = resNextIndex[i1];
            resNextIndex[i1]++;
            setpos(i1, i2, poss[i1], w);
            poss[i1](axis2) += grid[axis2][i2] + mSpacing[axis2];
        }
    }
    else
    {
