...
public:
    enum class TextBoxUnitType
    {
        Default, Percent, Angle, Num
    };
    ...
    void setSpinnable(bool spinnable) { mSpinnable = spinnable; markDity(); }

    const std::string &value() const { return mValue; }
    void setValue(const std::string &value) { mValue = value; markDity(); }

    const std::string &defaultValue() const { return mDefaultValue; }
    void setDefaultValue(const std::string &defaultValue) { mDefaultValue = defaultValue; markDity(); }

    Alignment alignment() const { return mAlignment; }
    void setAlignment(Alignment align) { mAlignment = align; markDity();  }

    const std::string &units() const { return mUnits; }

    TextBox* setUnits(const std::string &units);
    
    Widget* setUnitPercent();
    Widget* setUnitAngle();
    Widget* setUnitNum();

    int unitsImage() const { return mUnitsImage; }
    void setUnitsImage(int image) { mUnitsImage = image; markDity(); }
    ...
    void setFormat(const std::string &format) { mFormat = format; markDity(); }
    ...
    virtual void draw(NVGcontext* ctx, bool drawOnlyGui) override;
...
public:
    ...
    void setValueIncrement(Scalar incr) {
        mValueIncrement = incr; markDity();
    }
    void setMinValue(Scalar minValue) {
        mMinValue = minValue; markDity();
    }
    void setMaxValue(Scalar maxValue) {
        mMaxValue = maxValue; markDity();
    }
...
public:
    ...
    void numberFormat(const std::string &format) { mNumberFormat = format; markDity(); }
