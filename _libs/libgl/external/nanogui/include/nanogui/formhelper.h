...
public:
    ...
    addVariable(const std::string &label, const std::function<void(Type)> &setter, const std::function<Type()> &getter, bool editable = true) 
    {
        ...
        widget->setFixedSize(Vector2i(fs.x() != 0 ? fs.x() : mFixedSize.x(), fs.y() != 0 ? fs.y() : mFixedSize.y()));
    ...
    /// Add a new data widget that exposes a raw variable in memory
    template <typename Type> detail::FormWidget<Type> *
    addVariable(const std::string &label, Type &value, const std::function<void()> onValueChangedFromGUI, bool editable = true)
    {
        auto widget = addVariable<Type>(label,
            [&value, onValueChangedFromGUI](Type v)
        {
            value = v;
            onValueChangedFromGUI();
        },
            [&]() -> Type
        {
            return value;
