...
IGL_INLINE void igl::viewer::OpenGL_state::set_data(const igl::viewer::ViewerData &data, bool invert_normals)
{
  ...
  if (dirty & ViewerData::DIRTY_POSITION) V_vbo.transposeInPlace();
  if (dirty & ViewerData::DIRTY_NORMAL) V_normals_vbo.transposeInPlace();
  if (dirty & ViewerData::DIRTY_AMBIENT) V_ambient_vbo.transposeInPlace();
  if (dirty & ViewerData::DIRTY_DIFFUSE) V_diffuse_vbo.transposeInPlace();
  if (dirty & ViewerData::DIRTY_SPECULAR) V_specular_vbo.transposeInPlace();
  if (dirty & ViewerData::DIRTY_UV) V_uv_vbo.transposeInPlace();
  if (dirty & ViewerData::DIRTY_OVERLAY_LINES) lines_V_vbo.transposeInPlace();
  if (dirty & ViewerData::DIRTY_OVERLAY_LINES) lines_V_colors_vbo.transposeInPlace();
  if (dirty & ViewerData::DIRTY_OVERLAY_POINTS) points_V_vbo.transposeInPlace();
  if (dirty & ViewerData::DIRTY_OVERLAY_POINTS) points_V_colors_vbo.transposeInPlace();
  if (dirty & ViewerData::DIRTY_FACE) F_vbo.transposeInPlace();
  if (dirty & ViewerData::DIRTY_OVERLAY_LINES) lines_F_vbo.transposeInPlace();
  if (dirty & ViewerData::DIRTY_OVERLAY_POINTS) points_F_vbo.transposeInPlace();
#endif
}
...
{
  glDrawElements(GL_LINES, lines_F_vbo.size(), GL_UNSIGNED_INT, 0);
}

IGL_INLINE void igl::viewer::OpenGL_state::draw_overlay_points()
{
  glDrawElements(GL_POINTS, points_F_vbo.size(), GL_UNSIGNED_INT, 0);
}
