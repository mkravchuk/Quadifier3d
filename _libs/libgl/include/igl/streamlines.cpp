...
IGL_INLINE void igl::streamlines_init(
                                      ...
                                      bool match_with_curl, bool isFieldVectorsSorted
                                      ){
  ...
  for (unsigned i = 0; i < F.rows(); ++i){
    ...
    igl::sort_vectors_ccw(temp, n, order);
   /* if (order[0] != 0
        || order[1] != 1
        || order[2] != 2
        || order[3] != 3)
    {
        std::cout << "order_a is sorted " << order[0] << order[1] << order[2] << order[3] << std::endl;
    }*/
    
    // project vectors to tangent plane
    for (int j = 0; j < degree; ++j)
    {
      Eigen::RowVector3d pd = temp.segment(order[j] * 3, 3);
      pd = (pd - (n.dot(pd)) * n).normalized();
      data.field.block(i, j * 3, 1, 3) = pd;
    }
  }
  Eigen::VectorXd curl;
  igl::polyvector_field_matchings(data.field, V, F, data.E, FN, data.E2F, match_with_curl, treat_as_symmetric, data.match_ab, data.match_ba, curl, true);
...
IGL_INLINE void igl::streamlines_next(
  ...
  for (int i = 0; i < degree; ++i)
  {
    for (int j = 0; j < nsample; ++j)
    {
      ...
      //cout << "i=" << i << ",  p=" << p(0) << "," << p(1) << "," << p(2) << "," << "    r = " << r(0) << ", " << r(1) << ", " << r(2) << endl;
      ...
      for (int k = 0; k < 3; ++k)
      {
        ...
        if (igl::segments_intersect(p, r, q, s, t, u))
        {
          ...
          //cout << "f0=" << f0 << ",  f1=" << f1 << "    m0=" << m0 << ",m1=" << m1 << endl;
          break;
        }
