...
bool key_down(igl::viewer::Viewer& viewer, unsigned char key, int modifier)
{
  ...
  if (key == '4')
  {
    ...
    for (unsigned i=0; i<singularityIndex.size();++i)
    {
      if (singularityIndex(i) < 2 && singularityIndex(i) > 0)
        ...
        viewer.data.add_points(V.row(i),Eigen::RowVector3d(0,0,1));
    }
  ...
  /*Eigen::MatrixXd bndColor = Eigen::MatrixXd::Constant(F.rows(), 3, 1);
  for (unsigned i = 0; i<bnd.size(); ++i)
	  bndColor.row(bnd(i)) << 1, 0, 0;
...
int main(int argc, char *argv[])
{
  ...
  igl::readOBJ("K:/Mesh/TestObj/qq1_low.obj", V, F);
  //igl::readOBJ("K:/Mesh/TestObj/korzyna_small_1_medium.obj", V, F);

  //igl::readOBJ(TUTORIAL_SHARED_PATH "/circle.obj", V, F);
  ...
  // Create a smooth 4-RoSy field
  VectorXd S;
  igl::copyleft::comiso::nrosy(V, F, VectorXi(), MatrixXd(), b,  bw, bc, 4, 0.5, X1, S);
