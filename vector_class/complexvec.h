...
public:
    // default constructor
    Complex2f() { } // constructor should be empty - to avoid useless initialization - speed optimization by 5%
    ...
    Complex2f(complex<float> const * p)
    {
...
static inline Complex2f operator * (Complex2f const & a, double b)
{
    float f = (float)b;
...
static inline Complex2f & operator *= (Complex2f & a, double & b)
{
    a = a * b;
    return a;
}
...
public:
    // default constructor
    Complex4f() {} // constructor should be empty - to avoid useless initialization - speed optimization by 5%

    Complex4f(complex<float> const * p)
    {
        xmm = Vec4f().load(reinterpret_cast<float const*>(p));
    }
    Complex4f(char const * p)
    {
        xmm = Vec4f().load(reinterpret_cast<float const*>(p));
    }
    Complex4f(Complex4f const * p)
    {
        xmm = Vec4f().load(reinterpret_cast<float const*>(p));
    }
    Complex4f(complex<float> const * p1, complex<float> const * p2)
    {
    ...
    Complex4f(const complex<float>& c1, const complex<float>& c2)
    {
        xmm = Vec4f(c1.real(), c1.imag(), c2.real(), c2.imag());
    }
    ...
    // Assignment operator to convert from type __m128 used in intrinsics:
    //Complex4f & operator = (Complex2f const & x)
    //{
    //    xmm = Vec4f(x.real(), 0.f, x.imag(), 0.f);
    //    return *this;
    //}
    // Assignment operator to convert from type __m128 used in intrinsics:
    Complex4f & operator = (float const & re)
    {
        xmm = Vec4f(re, 0.f, re, 0.f);
        return *this;
    }
    // Assignment operator to convert from type __m128 used in intrinsics:
    Complex4f & operator = (complex<float> const & x)
    {
    ...
    {
        Complex2f(xmm).store(reinterpret_cast<float*>(p));
    }
    ...
    // change sign of imaginary part
    Complex4f getconjugated() const
    {
        return change_sign<0, 1, 0, 1>(xmm);
    }

    // change sign of real part
    Complex4f getconjugatedReal() const
    {
        return change_sign<1, 0, 1, 0>(xmm);
    }

    // from {real0, img0, real1, img1} return {real0, real0, real1, real1}
    Complex4f getOnlyReals() const
    {
        return Complex4f(_mm_moveldup_ps(xmm));
    }

    // from {real0, img0, real1, img1} return {img0, img0, img1, img1}
    Complex4f getOnlyImgs() const
    {
        return Complex4f(_mm_movehdup_ps(xmm));
    }

    // from {real0, img0, real1, img1} return {img0, real0, img1, real1}
    Complex4f getflipRealAndImg() const
    {
        return Complex4f(_mm_shuffle_ps(xmm, xmm, 0xB1));
    }
    ...
    Complex2f real() const
    {
        float x[4];
        Vec4f(xmm).store(x);
        return Complex2f(x[0], x[2]);
    }
    Complex2f imag() const
    {
        float x[4];
        Vec4f(xmm).store(x);
        return Complex2f(x[1], x[3]);
    }
...
public:
    ...
    Complex4fReal()
    {
    } // constructor should be empty - to avoid useless initialization - speed optimization by 5%

    //Complex4fReal(complex<float> const * p)
    //{
    //    xmm = Vec4f().load(reinterpret_cast<float const*>(p));
    //}
    //Complex4fReal(char const * p)
    //{
    //    xmm = Vec4f().load(reinterpret_cast<float const*>(p));
    //}
    //Complex4fReal(Complex4fReal const * p)
    //{
    //    xmm = Vec4f().load(reinterpret_cast<float const*>(p));
    //}
    // construct from real
    Complex4fReal(float re0, float re1)
    {
        xmm = Vec4f(re0, re0, re1, re1);
    }
    // construct from real
    Complex4fReal(float re)
    {
        xmm = Vec4f(re, re, re, re);
    }
    Complex4fReal(const Complex4f & c)
    {
        xmm = Complex4fReal(c[0], c[2]).xmm;
    }    
    Complex4fReal(__m128 const & x)
    {
        xmm = x;
    }
    Complex4fReal(Vec4f const & x)
    {
        xmm = x;
    }
    // Assignment operator to convert from type __m128 used in intrinsics:
    //Complex4fReal & operator = (__m128 const & x)
    //{
    //    xmm = x;
    //    return *this;
    //}
    Complex4fReal & operator = (const Complex4f & c)
    {
        xmm = Complex4fReal(c[0], c[2]).xmm;
        return *this;
    }
    ...
    operator Complex4f() const
{
    return Complex4f(xmm.m128_f32[0], 0, xmm.m128_f32[2], 0);
}
    ...
    //}
    // Member function to load from array (unaligned)
    //Complex4fReal & load(float const * p)
    //{
    //    xmm = Vec4f().load(p);
    //    return *this;
    //}
    //// Member function to load from array, aligned by 16
    //Complex4fReal const & load_a(float const * p)
    //{
    //    xmm = Vec4f().load_a(p);
    //    return *this;
    //}
    ...
    //void store(Complex4fReal * p) const
    //{
    //    Vec4f(xmm).store(reinterpret_cast<float*>(p));
    //}
    //void store(complex<float> * p) const
...
static inline Complex4fReal operator + (Complex4fReal const & a, Complex4fReal const & b)
{
    return Complex4fReal(Vec4f(a.xmm) + Vec4f(b.xmm));
}
...
// operator -= : subtract
static inline Complex4fReal & operator -= (Complex4fReal & a, Complex4fReal const & b)
{
    a = a - b;
    return a;
}
...
// operator == : returns true if a == b
static inline bool operator == (Complex4fReal const & a, Complex4fReal const & b)
{
    bool res = horizontal_and(Vec4f(a.xmm) == Vec4f(b.xmm));
    return res;
}
...
static inline bool operator != (Complex4f const & a, Complex4f const & b)
{
    Vec4fb t1 = Vec4f(a) != Vec4f(b);
    Vec4fb t2 = _mm_shuffle_ps(t1, t1, 0xB1); // swap real and imaginary parts
...
static inline Complex4fReal & operator -= (Complex4fReal & a, Complex4f & b)
{
    a = a - Complex4fReal(b);
    return a;
}
...
static inline Complex4f operator * (Complex4f const & a, Complex4fReal const & b)
{
    return _mm_mul_ps(a, b.xmm);
}
static inline Complex4f operator * (float a, Complex4f const & b) {
    return b * a;
}
static inline Complex4f operator * (Complex4fReal const & b, Complex4f const & a)
{
    return _mm_mul_ps(a, b.xmm);
}
static inline Complex4f & operator *= (Complex4f & a, float & b) {
...
static inline Complex4f operator / (Complex4f const & a, Complex4fReal const &  b)
{
    return _mm_div_ps(a, b.xmm);
} 
static inline Complex4fReal operator / (Complex4fReal const & a, Complex4fReal const &  b)
{
    return _mm_div_ps(a.xmm, b.xmm);
}
static inline Complex4fReal operator / (float a, Complex4fReal const &  b)
{
    return Complex4fReal(_mm_div_ps(_mm_set1_ps(a), b.xmm));
}
static inline Complex4f operator / (float a, Complex4f const & b) {
    Vec4f b2(b);
    Vec4f b3 = b2 * b2;
...
static inline Complex4f absPow2fast(Complex4f const & a)
{
    Vec4f a1 = Vec4f(a);
    Vec4f a2 = a1 * a1;
    Vec4f t1 = _mm_shuffle_ps(a2, a2, 0xB1); // swap real and imaginary parts
...
public:
    ...
    Complex8f(complex<float> const * p)
    {
        y.load(reinterpret_cast<float const*>(p));
    }
    ...
    void conjugate()
    {
        y = change_sign<0, 1, 0, 1, 0, 1, 0, 1>(y);
    }
};
